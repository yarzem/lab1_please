...
bool Json::is_array() const {
    ...
    for (unsigned i = 0; i < _json_string.length(); ++i) {
        ...
        if (!is_empty(symbol)){
            ...
            if (symbol == '\"') quotes++;
            if (((l_figure_brace == 0) and (symbol != '{')) and ((l_square_brace == 0) and (symbol != '['))) return false;
            if ((symbol=='[')and(l_square_brace==1)and(mas_type=-1))                //0-bool, 1-str, 2-mas, 3-obj, 4-int, 5-error
            {
                unsigned j=i+1;
            ...
            if (symbol!=','){
                switch (mas_type){
                    ...
                    case 1 : {
                        if ((quotes%2==0)and(!((symbol==',')or(symbol==']')or(symbol=='\"')))) return false;
                        if (quotes%2==0){
                            unsigned j=i+1;
                            while ((j<_json_string.length())and(is_empty(_json_string[j]))) { j++; }
                            if ((_json_string[j]!=',')and(_json_string[j]!=']')) return false;
        ...
        if (!is_empty(symbol)) {
            ...
            if ((quotes%2==0) and (symbol==':')){
                ...
                if (ch=='\"') {
                    ...
                    if ((_json_string[k+1]!=',') and (_json_string[k+1]!=']') and (_json_string[k+1]!='}')) return false;
                }
                if ((ch != '\"') and (ch != '{') and (ch != '[')) {
...
void Json::print() {
    if (_json_vector.empty()) for (auto it = _json_map.begin(); it != _json_map.end(); ++it){
        std::cout << it->first << ": ";
